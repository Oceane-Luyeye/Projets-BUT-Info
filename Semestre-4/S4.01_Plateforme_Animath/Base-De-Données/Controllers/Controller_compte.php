...
class Controller_compte extends Controller
{
    ...
    public function action_update_p() {
        ...
        if (isset($_POST["etablissement"]) and isset($_POST["ville"]) and isset($_POST["niveau"]) and isset($_POST["nb"])) { //a ce niveau là le niveau a mettre dans les champs à remplir
            $infos = [];
            $noms = ['etablissement', 'ville', 'niveau', 'nb'];  //a ce niveau là le niveau a mettre dans les champs à remplir
            foreach ($noms as $v) {
                if (isset($_POST[$v])) {
                    $infos[$v] = $_POST[$v];
                }
